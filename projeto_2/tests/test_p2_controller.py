...
def test_handle_mapa_obter_endereco_com_offset():
    """Garante que a conversão de pixels para grid respeita os offsets de centralização."""
    mapa = MapaQuadrado(10, 10)
    handler = HandleMapa(mapa)
    ...
    assert handler.obter_endereco_pela_posicao(100, 100, offset_x=off_x, offset_y=off_y) == (0, 0)
    
    # Clique em (131, 131) pixels -> (0, 0) grid
    assert handler.obter_endereco_pela_posicao(131, 131, offset_x=off_x, offset_y=off_y) == (0, 0)
    
    # Clique em (132, 132) pixels -> (1, 1) grid
    assert handler.obter_endereco_pela_posicao(132, 132, offset_x=off_x, offset_y=off_y) == (1, 1)
