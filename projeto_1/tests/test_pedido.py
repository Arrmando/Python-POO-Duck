...
def test_item_pedido_receita_nula_deve_levantar_erro():
    with pytest.raises(
        ValueError,
        match="O item da receita precisa estar vinculado a uma receita válida",
    ):
...
def test_item_pedido_coeficiente_invalido_deve_levantar_erro():
    ...
    with pytest.raises(
        ValueError, match="O coeficiente do item deve ser maior que zero"
    ):
...
def test_pedido_adicionar_item_invalido_deve_levantar_erro():
    ...
    with pytest.raises(
        ValueError, match="Apenas objetos do tipo ItemPedido podem ser adicionados"
    ):
