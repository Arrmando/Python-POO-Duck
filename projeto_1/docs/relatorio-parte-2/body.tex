...
\begin{itemize}
  \item \texttt{InsumoController} (classe do framework) — recebe a requisição e coordena o fluxo;
  \item \texttt{CriarInsumoDTO} (classe do framework) — representa e valida a estrutura do corpo da requisição;
...
\texttt{Ingrediente} e \texttt{HomemHora} que fazem parte do workflow de criar insumos.

\lstinputlisting[
  language=Python,
  caption={Implementação da classe Ingrediente},
  label={list:ingrediente}
]{../../src/projeto_1/dominio/ingrediente.py}

\lstinputlisting[
  language=Python,
  caption={Implementação da classe HomemHora},
  label={list:homem-hora}
]{../../src/projeto_1/dominio/homem_hora.py}
